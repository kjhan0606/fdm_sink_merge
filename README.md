...
The host-demographic calculation measures the number density of active SMBH
pairs in distinct PSB galaxies and estimates its uncertainty by omitting eight
spatial regions in turn. It also fits the modified Schechter-like function used
for the full spatial sample. The companion-sensitivity calculation successively
requires a unique assigned companion and a relative speed below the escape
speed generated by the two SMBH point masses. The strict phase-space condition
is a diagnostic because it omits the host potential and unresolved matter.
The validation command checks the host counts, fitted populations, nested
companion selections, matched host sample, and all three publication figures.
